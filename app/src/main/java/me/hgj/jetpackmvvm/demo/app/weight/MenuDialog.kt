...
class MenuDialog {
    ...
    class Builder(context: Context) : BaseDialog.Builder<Builder>(context),
        ...
        fun setList(data: MutableList<out Any>): Builder = apply {
            adapter.setData(data as MutableList<BankCardResponse>)
    ...
    class MenuAdapter (context: Context) : AppAdapter<BankCardResponse>(context) {
        ...
        inner class ViewHolder : AppViewHolder(R.layout.menu_item) {
            ...
            override fun onBindView(position: Int) {
                val bankCardResponse = getItem(position)
                val takeLast = bankCardResponse.cardNo?.takeLast(4)?:""

                textView?.text = "${bankCardResponse.bank}($takeLast)"
                if (position == 0) {
                    // 当前是否只有一个条目
                    if (getCount() == 1) {
